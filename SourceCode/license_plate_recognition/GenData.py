...
def main():
    ...
    for f in os.listdir(super_path_new_data):
      ...
      for img in os.listdir(sub_path_new_data):
        image_path=sub_path_new_data+img
        # print(image_path)
        imgTrainingNumbers = cv2.imread(image_path)            # read in training numbers image
        imgTrainingNumbers=cv2.resize(imgTrainingNumbers,(150,200))
    ...
    # end if
        ...
        for npaContour in npaContours:                          # for each contour
          if cv2.contourArea(npaContour) > MIN_CONTOUR_AREA:          # if contour is big enough to consider
            ...
            # skel = cv2.dilate(skel,kernel_1,iterations = 2)
            ...
            # cv2.moveWindow('imgROI', 240, 240)
            # cv2.imshow("imgROIResized", imgROIResized)      # show resized image for reference
            # cv2.moveWindow('imgROIResized', 480, 240)
            ...
            # cv2.moveWindow('imgTrainingNumbers', 700, 240)
            # cv2.waitKey()
            intChar = ord(name)
            # cv2.destroyAllWindows()                     # get key press
            # print(intChar)
